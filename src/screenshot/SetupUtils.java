...
public class SetupUtils {
	...
	public boolean createLnk(String appName) {
		try {
			...
			// 指向的exe
			// 注意可能自己生成的exe名字带有版本号，所以提前预测下
			link.setPath(savePath + f.separator + appName);
			link.save();
			return true;
		} catch (Throwable e) {
			// 是更改后的jar应用，直接全部抛出
	...
	public String getExeAppName() {
		...
		for (int i = 0; i < fs.length; i++) {
			if (fs[i].isDirectory() || !fs[i].getName().endsWith(".exe") || !fs[i].getName().startsWith("ScreenShot")) {
				continue;
			} else {
				appName = fs[i].getName();
				break;
			}
		}
		return appName;
